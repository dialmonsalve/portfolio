...
export class DialDropZone extends HTMLElement {
  static paragraph: HTMLParagraphElement;
  constructor() {
    super();
    ...
    dropZones.forEach((dz) => {
      ...
      DialDropZone.paragraph = this?.querySelector(
        "#lbl-selected-files"
  ...
  static onInputChange = (evt: Event, dropZone: HTMLLabelElement) => {
    ...
    if (files) {
      DialDropZone.handleFiles(files, target);
  ...
  static onDrop = async (
    ...
    if (files) {
      DialDropZone.handleFiles(files, fileInput);
  ...
  static handleFiles = (files: FileList, fileInput: HTMLInputElement) => {
    ...
    this.paragraph.innerHTML = `<strong>${validFiles.length} archivos seleccionados</strong>`;
